...
class TwoLayerNet(object):
  ...
  The outputs of the second fully-connected layer are the scores for each class.
    ...
    #############################################################################
        ...
        cor_scores = np.choose(y,shift_scores.T)
        loss = -cor_scores + np.log(np.sum(np.exp(shift_scores),axis=1))
        loss = np.sum(loss)/N + reg*(np.sum(W1*W1)+np.sum(b1*b1)+np.sum(W2*W2)+np.sum(b2*b2))
    ...
    #############################################################################
        ...
        dSoft /=N
        #Backprop dW2
        dW2 = np.dot(relu_1_activation.T,dSoft) + 2*reg*W2
        grads['W2'] = dW2
        #Backprop db2
        db2 = dSoft
    ...
    # Use SGD to optimize the parameters in self.model
        ...
        for it in range(num_iters):
            batch_ind = np.random.permutation(num_train)[0:batch_size]
            X_batch = X[batch_ind,:]
            y_batch = y[batch_ind]
      #########################################################################
